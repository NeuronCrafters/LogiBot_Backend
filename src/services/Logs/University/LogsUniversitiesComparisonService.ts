...
export async function LogsUniversitiesComparisonService(
  ...
  const university1Data = await fetchUniversityData(universityId1);
  const university2Data = await fetchUniversityData(universityId2);
